...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // 1. Check for Local Session (Teachers/Students)
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      } else if (!localStorage.getItem('eduassist_local_session')) {
    ...
    if (currentUser.role === 'TEACHER') {
      ...
      const autoTasks: Task[] = teacherLessonPlans
        ...
        .map(lp => ({
          ...
          name: `Prepare: ${teacherClasses.find(c => c.id === lp.classId)?.name || 'Lesson'} (${lp.date})`,
          dueDate: lp.date,
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black theme-primary uppercase tracking-widest">Initializing EduAssist...</div>;
  ...
  const renderContent = () => {
      ...
      switch (activeMenu) {
        case 'dashboard':
          return <TeacherDashboard 
            ...
            onClassClick={(id) => { setSelectedClassId(id); setActiveMenu('classes'); }}
            onTaskClick={() => setActiveMenu('tasks')}
          />;
        case 'classes':
          if (selectedClassId) {
            ...
            return cls ? (
              <ClassDetails 
                ...
                onDeletePlan={async (id) => await deleteDoc(doc(db, 'lessonPlans', id))}
                updateClass={async (updated) => await setDoc(doc(db, 'classes', updated.id), updated)}
              />
            ) : null;
          }
    ...
    if (currentUser.role === 'STUDENT' && filteredData) {
      return <StudentDashboard student={currentUser} classes={filteredData.classes} attendance={filteredData.attendance} examResults={filteredData.examResults} />;
    }

    return <div className="p-20 text-center font-bold text-slate-400">UNAUTHORIZED ACCESS</div>;
  };
